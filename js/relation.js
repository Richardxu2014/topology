    ...
    CloudRelation.prototype = {
        ...
        _init: function () {
            ...
            Raphael.fn.connection = function (obj1, obj2, nodeOption) {
                ...
                if (line && line.line) {
                    ...
                    return {
                        line: this.path(path).toBack().attr({ 'stroke': cfg.lintcolor, 'stroke-width': cfg.lintwidth, fill: "none" }),
                        from: obj1,
                        to: obj2,
                        lineNode: self.paper.set().push(this.circle(poc.x, poc.y, cfg.middlePiont).attr({ fill: cfg.middlePiontFill, stroke: cfg.middlePiontStroke }), this.text(poc.x, poc.y, cfg.middlePiontText).attr({ stroke: cfg.middlePiontTextColor, cursor: 'pointer' })),
                        lineOpt: nodeOption
                    };
                }
            };
        ...
        render: function (data) {
            ...
            function drawSingleNode(node, pos, rad, angle) {
                ...
                var nodeShape;
                if (cfg.mode == 'edit') {
                    nodeShape = drawShape('1', p, circleRadius).attr({ 'fill': node.color, 'stroke': cfg.nodestrokecolor, 'stroke-width': cfg.nodestrokewidth }).data('setIndex', idx);
                } else {
